...
$custmer_id=$_REQUEST['custmer_id'];
$sql="select * from staff_details where staff_id=$staff_id and id=$id";
...
$sql1="select * from order_detail join add_product on order_detail.product_id=add_product.product_id where order_detail.customer_id='$custmer_id' and add_product.id=$id";
...
if($count1>0){
	...
	if($count>0){
		...
		// $up="update staff_details set status='Busy' where staff_id=$staff_id and id='$id'";
		// $r=$con->query($up);
	...
	while($row1=$result1->fetch_assoc()){
		...
		$msg1[]=array("product_id"=>$product_id,"product_name"=>$product_name,"brand"=>$brand,"rack_no"=>$rack_no,"price"=>$price,"quantity"=>$qu);
...
// $msg = array
// (
// 	'message' 	=> 'Customer Order Details From Admin',
...
$msg= array('Customer_id'=>$data1,"Reciever"=>"Staff");
//print_r($msg);die();
